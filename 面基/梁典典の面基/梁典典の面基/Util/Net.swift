        ...
        provider.request(target) { result in
            ...
            case let .success(response):
                do {
                    ...
                    default:
                        print("\(responseObject["data"].description)")
...
enum API {
    ...
    //申请面基
    case addMianji(model: AddMianjiParams)
...
extension API: TargetType {
    ...
    var path: String {
        ...
        case .addMianji(model: _):
            return "/api/mianji/add"
        }
    }
    ...
    var task: Task {
        ...
        case .getCategorys(let name):
            params["name"] = name
        case .addMianji(let model):
            let json = JSON(model.parameters)
